...
<template>
  <v-container>
    <v-card class="mx-auto w-auto">
      Result of <code>https://dummyjson.com/products/</code>
      <input v-model="id" type="number" />
    <hr />
      <v-btn variant="tonal" :disabled="!canDecrease" @click="canDecrease ? id-- : null">
        Previous
      </v-btn>
      -
      <v-btn variant="tonal" @click="id++">
        Next
      </v-btn>
    </v-card>
    ...
    <div class="grid grid-cols-3 gap-4" v-else>
        <div class="col-span-1">
            <a :href="product.images" target="_blank"><img :src="product.thumbnail" /></a>
